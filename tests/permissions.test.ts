...
import { can, canAccessPortal, roleHome } from "../lib/permissions.ts";
...
test("portal access requires an exact role match",()=>{assert.equal(canAccessPortal("customer","customer"),true);assert.equal(canAccessPortal("admin","admin"),true);assert.equal(canAccessPortal("admin","customer"),false);assert.equal(canAccessPortal("customer","admin"),false);assert.equal(canAccessPortal("technician","customer"),false)});
